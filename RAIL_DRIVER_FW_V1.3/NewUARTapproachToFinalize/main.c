...
void initUSART() {
    ...
    // USART module configuration (BRG 16-bit for precise baud and higher rates)
    TXSTAbits.SYNC = 0;     // Asynchronous
    TXSTAbits.BRGH = 1;     // High speed (Fosc/(4*(SPBRG+1)))
    ...
    TXSTAbits.TXEN = 1;     // Enable transmitter (after SPBRG set)

    gl_receivedUSARTPointer=0;
    gl_getDataUSARTPointer=0;

    PIE1bits.RCIE = 1;      // Enable USART RX interrupt
    PIE1bits.TXIE = 0;      // TX interrupt off by default (enabled on send)
    IPR1bits.TXIP = 0;   // UART TX = low priority
    IPR1bits.RCIP = 1;   // (optional) UART RX = high priority
    IPR3bits.RXB0IP = 1; // CAN RXB0 = high priority
    IPR3bits.RXB1IP = 1; // CAN RXB1 = high priority
}
/*****************************************************************************/
/* sendUSART */
/*****************************************************************************/
void sendUSART(unsigned char data){
	...
	gl_txHead = next;
	PIE1bits.TXIE = 1; // ensure TX interrupt is on

	gl_txBuf[gl_txHead] = data;
	gl_txHead = next;

	// Kick TX interrupt to start/continue sending
	PIE1bits.TXIE = 1;
}
/*****************************************************************************/
...
void low_isr(void){
    ...
    // UART TX interrupt: send next byte if available (low priority)
    if (PIE1bits.TXIE && PIR1bits.TXIF){
        if (gl_txHead != gl_txTail){
            TXREG = gl_txBuf[gl_txTail++];
            if (gl_txTail >= TXBUFFERSIZE) gl_txTail = 0;
        } else {
            PIE1bits.TXIE = 0; // nothing left to send
        }
    }
