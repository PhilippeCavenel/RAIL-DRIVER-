...
void DefaultConnectionPainter::drawNormalLine(QPainter *painter, ConnectionGraphicsObject const &cgo) const
{
    ...
    if (useGradientColor) {
        ...
        // Utiliser le style personnalisé de ConnectionGraphicsObject
        qreal highlightWidth = cgo.highlightLineWidth();
        QColor highlightColor = cgo.highlightColor();

        // On utilise ces valeurs uniquement si elles sont différentes du style par défaut
        if (highlightColor != QColor("gray")) {
            p.setColor(highlightColor);
            p.setWidthF(highlightWidth);
        } else {

            // Prend le style défini par setHighlight()
            QColor overrideColor = cgo.highlightColor();
            qreal overrideWidth = cgo.highlightLineWidth();

            bool isCustomStyle = (overrideColor != QColor("gray") || overrideWidth != 3.0);

            if (isCustomStyle) {
                p.setColor(overrideColor);
                p.setWidthF(overrideWidth);
            } else {
