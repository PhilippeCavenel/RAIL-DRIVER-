...
#include <ui_InnoMakerMainWindow.h>
#include <QDebug>
#include <QMetaType>
#include <QMessageBox>
#include <worker.h>
...
InnoMakerMainWindow::InnoMakerMainWindow(QWidget *parent)
    : QMainWindow(parent)
...
InnoMakerMainWindow::~InnoMakerMainWindow()
{
...
void InnoMakerMainWindow::on_pushButton_scanDevice_clicked()
{
...
void InnoMakerMainWindow::on_pushButton_openDevice_clicked()
{
    if(curDevice != NULL) {
    ...
    connect( worker, &Worker::error, this, &InnoMakerMainWindow::errorString);
...
void InnoMakerMainWindow::on_pushButton_delaysend_clicked()
{
    ...
    connect( delaySender, &DelaySender::error, this, &InnoMakerMainWindow::errorString);
    connect( delaySender, &DelaySender::finished, this, &InnoMakerMainWindow::delaySenderFinish);
...
int InnoMakerMainWindow::bytesToInt(unsigned char* bytes, int size)
{
    int a = bytes[0] & 0xFF;
    a |= ((bytes[1] << 8) & 0xFF00);
...
int InnoMakerMainWindow::bytesToInt1(unsigned char* bytes, int size)
{
    int a = bytes[3] & 0xFF;
    a |= ((bytes[2] << 8) & 0xFF00);
...
void InnoMakerMainWindow::on_pushButton_send_clicked()
{
...
void InnoMakerMainWindow::on_pushButton_clear_clicked()
{
...
void InnoMakerMainWindow::on_pushButton_export_clicked()
{
...
void InnoMakerMainWindow::send( MessageA msg) {

}

void InnoMakerMainWindow::recv( Message msg) {
    ...
    ///If receiving passively, scroll directly to the bottom.
    if(delaySender == NULL ) {
    ...
    /// If actively sending, and the baud rate is too high with very short intervals, use a counter-based refresh mechanism to prevent the UI thread from freezing.
    int interval = this->ui->lineEdit_sendInterval->text().toInt();
    int frameToSend = this->ui->lineEdit_numToSend->text().toInt();
...
InnoMakerUsb2CanLib::Innomaker_device_bittming InnoMakerMainWindow::getBittming(int index) {
...
void InnoMakerMainWindow::errorString(QString str) {

}


void InnoMakerMainWindow::on_lineEdit_frameId_textEdited(const QString &arg1)
{
    formatter->adjustTextFieldToHex(ui->lineEdit_frameId,8,true);
}


void InnoMakerMainWindow::on_lineEdit_data_textEdited(const QString &arg1)
{
    formatter->adjustTextFieldToHex(ui->lineEdit_data,16,true);
}

void InnoMakerMainWindow::saveToCSV()
{
...
void InnoMakerMainWindow::delaySenderFinish() {
    delaySender = NULL;
    sendThread = NULL;
    scrollToBottomCounter = 0;
    this->ui->pushButton_delaysend->setText("Delay Send");
...
bool InnoMakerMainWindow::nativeEvent(const QByteArray &eventType, void *message, long *result)
{
    Q_UNUSED(eventType);
